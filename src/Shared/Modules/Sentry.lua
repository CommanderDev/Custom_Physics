--[[
	...
	SentryClient:ConnectRemoteEvent(remoteEvent) with a RemoteEvent not in use by anything
	...
	Sentry:Client(string dsn[, table config])
		Creates a new Sentry client used to send events.
	...
	Sentry:SendMessage(string message[, string level = Sentry.EventLevel.Info][, table config])
		Sends plain message event to Sentry.
		
		@level: a string describing the severity level of the event
		Valid levels (in Sentry.EventLevel):
			Fatal
			Error
			Warning
			Info
	...
	Sentry:SendException(string ExceptionType, string errorMessage[, <string, table> traceback][, table config])
		Send exception event to Sentry.
		
		@ExceptionType: a string describing the type of exception.
		Provided exception types (in Sentry.ExceptionType):
	...
	level
		The record severity.
		Enumeration of valid levels in Sentry.EventLevel
	...
	local client = Sentry:Client("DSN here")
	...
	if (not success) then
		client:SendException(Sentry.ExceptionType.Server, err, debug.traceback())
	end
	
	client:SendMessage("Fatal error", Sentry.EventLevel.Fatal)
	client:SendMessage("Basic error", Sentry.EventLevel.Error)
	client:SendMessage("Warning message", Sentry.EventLevel.Warning)
	client:SendMessage("Info message", Sentry.EventLevel.Info)
	client:SendMessage("Debug message", Sentry.EventLevel.Debug)
	
	local LogService = game:GetService("LogService")
	LogService.MessageOut:Connect(function(message, messageType)
		if (messageType == Enum.MessageType.MessageError) then
			client:SendException(Sentry.ExceptionType.Server, message)
		end
	end
			...
			if (succeed) then
				...
				if (status) then
					if (status >= 400 and status < 500) then
						if (logWarnings) then
							warn(("Sentry: HTTP %d in TrySend, JSON packet:"):format(status))
							warn(packetJSON)
							warn("Headers:")
...
Sentry.ExceptionType = {
	Server = "ServerError";
	Client = "ClientError";
}

function Sentry:Client(dsn, config)
	local client = {}
...
function Sentry:SendMessage(message, level, config)
	config = config or {}
...
function Sentry:SendException(eType, errorMessage, traceback, config, eventLevel)
	...
	local packet = {
		level = eventLevel or Sentry.EventLevel.Error;
		culprit = culprit;
	}
...
function Sentry:ConnectRemoteEvent(remoteEvent)
	...
	remoteEvent.OnServerEvent:Connect(function(player, errorMessage, traceback, options)
		...
		if (count > 0) then
			if (type(errorMessage) == "string" and (type(traceback) == "string" or traceback == nil)) then
				...
				if (success) then
					count = count - 1
					self:SendException(Sentry.ExceptionType.Client, scrubbedErrorMessage, scrubbedTraceback, options)
				else
					if (logWarnings) then
						warn(("Sentry: Player '%s' tried to send spoofed data, their ability to report errors has been disabled."):format(player.Name))
						warn("errorMessage:")
						warn(errorMessage)
						warn("traceback:")
						warn(traceback)
